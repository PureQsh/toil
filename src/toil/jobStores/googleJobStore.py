...
class GoogleJobStore(AbstractJobStore):
    class Locator(namedtuple('Locator', ('projectID', 'namePrefix', 'headers', 'uri')),
        ...
        """
        Parses a job store locator string into a Google locator object containing the following
        attributes:
        ...
        def parse(cls, l):
            ...
            if len(l) not in (1, 2):
                ...
                raise ValueError("Invalid name prefix '%s'. Name prefixes cannot contain "
                  ...
                  GoogleJobStore.maxBucketNameLen -
                  len(GoogleJobStore.bucketSuffix) -
                  len('gs://')):
                ...
                uri = boto.storage_uri("gs://%s%s" % (prefix, GoogleJobStore.bucketSuffix),
                                       GOOGLE_STORAGE)
                return cls(projectID, prefix, headers, uri)

        def __str__(self):
            return ':'.join(('google', self.projectID or '', self.namePrefix))
    ...
    def cleanJobStore(cls, locator):
        ...
        bucket = None
        try:
            for attempt in retry(timeout=6, predicate=noSuchBucket):
                with attempt:
            ...
            while True:
                try:
                    ...
                    if e.status == 404:
                        ...
                        log_component_failure(e, log, locator.uri, 0)
                ...
                for obj in bucket.list():
                    try:
                        ...
                        pass
        log_delete_completed(log, locator, existed, False)
